...
def FilePicker( path , topLevel = False ):
	...
	while True:
		...
		elif keyIn == curses.KEY_HOME or keyIn == curses.KEY_PPAGE:
			selected = 0
		elif keyIn == curses.KEY_END or keyIn == curses.KEY_NPAGE:
			selected = len(dirFileList)-1
		## Select
			...
			if os.path.isfile(fullPath):
				##### LOAD FILES #####
				dictINF = {}
				dictINF['notes'] = fullPath
				with open(fullPath.replace('.txt','.inf')) as infFile:
					for line in infFile.readlines():
						(infKey,infValue) = line.strip().split('=')
						if infKey == 'videostart':
							dictINF[infKey] = infValue
						else:
							dictINF[infKey] = os.path.join( path, infValue )
						#screen.erase()
						#screen.addstr( 2, 2, infKey + ' ' + dictINF[infKey], normalText)
						#screen.refresh()
						#time.sleep(2)
				#fullPathVideo = dictINF['video']
				EditorLoop( dictINF )#, fullPathVideo )
...
def EditorLoop( dictINF ):
	## LOADING
	screen.erase()
	curses.resize_term(*screen.getmaxyx())
	curses.curs_set( 0 )
	# Load Pacenotes
	screen.addstr( 0, 1, 'Loading Pacenotes...' + dictINF['notes'], normalText)
	screen.refresh()
	pacenotes = _pacenotes.classPacenotes( dictINF['notes'], NoteResolution )
	# Load Map Trace
	screen.addstr( 1, 1, 'Loading Trace...' + dictINF['trace'], normalText)
	screen.refresh()
	trace = _trace.classTrace( dictINF['trace'], pacenotes.getNoteResolution() )
	# Load editor
	screen.addstr( 3, 1, 'Initialising plots and main editor...', normalText)
	screen.refresh()
	
	## Start video
	screen.addstr( 4, 1, 'Initialising video...', normalText)
	screen.addstr( 5, 1, ' ' + dictINF['video'], normalText)
	screen.refresh()
	_ffplayer.startPlayback( dictINF['video'], dictINF['videostart'] )
	...
	while True:
		...
		if videoPlayback:
########### RUNNING VIDEO
			# Check if Key Input, non-blocking with with delay to slow down updates and catch keys more reliably
			screen.timeout(500)
			keyIn = screen.getch()
			screen.timeout(0)
			## VIDEO CONTROLS
			if keyIn == -1:
				# No input
				# Must check for -1 or it crashes when trying to convert to keyname
				pass
			elif curses.keyname(keyIn).decode() == ' ':
				_ffplayer.togglePlayPause()
				videoPlayback = False
			# Skipping
			# TODO on the fly skipping distance
			#	skip based on distance, then I can easily set new distance in this loop, and pass the required time to ffplayer
			#	skip back/foward ?10m with cursor up/down - tune distance to something logical to replay a corner
			#	skip to next/previous note with pageup/down
			# VOLUME
			elif curses.keyname(keyIn).decode() in 'mM':
				_ffplayer.toggleMute()
			elif keyIn == curses.PADPLUS or curses.keyname(keyIn).decode() in '+=':
				_ffplayer.changeVolume(+0.1)
			elif keyIn == curses.PADMINUS or curses.keyname(keyIn).decode() in '-_':
				_ffplayer.changeVolume(-0.1)
			#
			##### Update DISTANCE from Player Time
			currTime = _ffplayer.getCurrTime()
			statusText = str(round(currTime,1))
			dist = trace.getDistFromTime(currTime)
			#
		else:
########### PAUSED STATE VIDEO
			curses.flushinp()	# flush input buffer to ignore keypresses while busy (so we don't keep scrolling after releasing keys)
			screen.nodelay(0)	# wait for input
			keyIn = screen.getch()
			### PROCESS KEYPRESS ###
			## Overall
			if keyIn == curses.KEY_RESIZE or curses.keyname(keyIn).decode() == 'KEY_F(5)':
				pass
			...
			# F12 = exit (no autosaving) (Don't use ESC, as ESC is used to abort editing call text)
				...
				_ffplayer.exitPlayback()
				return
			elif keyIn == 27:
				statusText = 'Use F12 to exit, make sure to save first!'
			#
			##### Update Player Time from Distance
			currTime = trace.getTimeFromDist(dist)
			_ffplayer.seekToTime(currTime)
#
...
# Start another thread to process video
_thread.start_new_thread(_ffplayer.videoLoop,())	# need to include empty tuple for arguments
